...
# Description:       starts the tomcat daemon
### END INIT INFO
...
CRONOLOG='/data/apps/deps/cronolog/sbin/cronolog'
...
test ".$CATALINA_OUT" = . && CATALINA_OUT="$CATALINA_BASE/logs/catalina-daemon.out.%Y-%m-%d"
...
case "$1" in
    ...
    start   )
      ...
      -Djava.io.tmpdir="$CATALINA_TMP" \
      $CATALINA_MAIN 2>&1 \
      | $CRONOLOG "$CATALINA_OUT" >> /dev/null &
      exit $?
    ;;
    stop    )
      "$JSVC" $JSVC_OPTS \
    ...
    status  )
      ...
      -check \
      $CATALINA_MAIN 2>&1 \
      | grep 'Still running according to PID file'
      exit $?
    ;;
    restart   )
      $0 stop
